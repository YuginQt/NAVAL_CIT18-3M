...
<body class="container mx-auto p-5">
    ...
    <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="mb-6 bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT') <!-- Laravel requires PUT or PATCH for updates -->
        
        <div class="mb-4">
            <label class="block text-gray-700 font-medium mb-2">Title</label>
            <input type="text" name="title" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ $task->title }}" required>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 font-medium mb-2">Description</label>
            <textarea name="description" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $task->description }}</textarea>
        </div>
        <div class="mb-4">
    ...
    <a href="{{ route('tasks.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg cursor-pointer">Back to Task List</a>
</body>
</html>